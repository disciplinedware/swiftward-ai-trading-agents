...
## MANDATORY: Every buy order MUST include `stop_loss` in params

Every call to `trade/submit_order` with `side: "buy"` must include `params.stop_loss` set to a numeric trigger price (e.g. 5% below entry). Orders without `stop_loss` are rejected by policy. No exceptions, no exploration, no "just trying without SL first". If you forget `stop_loss`, the order fails and the session wastes turns.
...
trade/submit_order {
  ...
  "params": {
    ...
    "take_profit": 2400,
    "strategy": "momentum",
    "reasoning": "1h EMA cross + rising volume",
    "confidence": 0.7
  }
}
```
...
```
trade/submit_order { "pair": "ETH-USD", "side": "buy", "value": 800 }
```

Before every buy, compute `stop_loss = entry_price * 0.95` (or tighter based on ATR) and include it. Do not submit a buy without it.
